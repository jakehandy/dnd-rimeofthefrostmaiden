# Z'Mora

Status: `Session 1 notes + confirmed visual reference`

## Identity (campaign facts)
- Name spelling in sources: `Z'Mora` / `Za'mora` (likely same character; confirm preferred spelling)
- Species: Tabaxi
- Role: Guide of Icewind Dale

## Session 1 confirmed facts
- Was approached by a frantic mother in Lonelywood claiming her daughter had been kidnapped by the White Moose.
- Accepted the job and enlisted Arjay/Arhjay and Hadrian.
- Tracked the White Moose, met Tar on the trail, and freed Uriel from the ritual site.

## Open identity unknowns
- Preferred canonical spelling: Unknown
- Class/build details: Unknown

## Canonical portrait
- Source image: `data/sources/character-portraits/2026-02-09_zmora.png`
- Original filename from user: `Z'Mora.PNG`

## Visual profile (portrait source)
- Snow-leopard feline humanoid (tabaxi-like) with blue eyes.
- Winter-white cloak/scarf and archer loadout.
- Wooden bow and green-fletched arrows.
- Low, stealthy hunter posture in snowy terrain.

## Keep consistent in generated scenes
- Distinct snow-leopard pattern and tail.
- Archer identity cues (bow, quiver, stalking stance).
- Cold-weather camouflage fabrics and muted tones.

## Prompt seed
"Z'Mora, snow leopard feline archer in white winter cloak, crouched in snow with bow and green-fletched arrows, painterly fantasy illustration"

## Story arc, Sessions 2-5 (source: 2026-05-19 player notes)
- S2: Kept watch at the cave mouth; found the Netherese **sky-city wall carvings**; **removed the black crystal** from the warded door, unsealing whatever lay behind it.
- S3: Heard the released creatures climbing the stairs and **readied the party for battle**; with Hadrian, destroyed most of the escaping horde.
- S4: Brought **Cara back to her mother**; received a family heirloom **horn of blasting** as payment.
- S5: With Arjay, sought out Speaker **Masthew** in Termalaine. Received **magical arrows** from Hadrian (bought cheap from an unaware shopkeeper). **Revived Tar** at the start of the session.
- Gear gained: **horn of blasting** (S4), **magical arrows** (S5).
- Throughline: the party's tracker/scout and ranged threat; reliable in a pinch (revives, recon, archery).

## Story arc, Session 6 (source: 2026-06-17 session recap)
- Called out by name: Vellyenne Harpell was visibly distressed to learn that **Za'mora** was the one who unleashed the horde of monstrosities from the Netherese moon-cave (by removing the black crystal in S2).
- Vellyenne examined Za'mora's black crystal and identified it as the most refined Chardalyn she's ever seen, warded with a spell — the party's clearest read yet on what that crystal actually is.
- Still holds the black crystal as of this session; its fate (kept, studied, or handed over) is an open thread.
