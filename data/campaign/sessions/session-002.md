...
- Uriel put on the gauntlet and tested two controls:
  - `suppress`: burns Uriel; stair glow turns off.
  - `sentry`: no burn; stair glow turns on.
- The party proceeded down the stairs after identifying control behavior.
- The party turned `sentry` back on after descending.
- Two giant statues activated, initiating combat.
- Initiative note: Tar rolled `8` and is acting fifth in party order.
- Tar currently has advantage on `Acrobatics` and `Athletics` (source shown in live sheet state).
- Combat log: enemy made multiple attacks into Tar; one missed, one hit for `17` damage (mitigation application Unknown).
- Encounter state changed to out of combat after Uriel used the switch.
